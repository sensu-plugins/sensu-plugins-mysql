  ...
  option :check, description: 'type of check: status | replication', short: '-c CHECK', long: '--check CHECK', default: 'status'
	...
	def run
		...
		if config[:check] == 'status'
			begin
				cmd = "#{config[:binary]} -u #{db_user} -h #{config[:hostname]} --port #{config[:port]} --socket #{db_socket} -p\"#{db_pass.strip}\" --batch --disable-column-names -e 'show schemas;'"
		...
		if config[:check] == 'replication'
			table = Hash.new
			begin
				cmd = "#{config[:binary]} -u #{db_user} -h #{config[:hostname]} --port #{config[:port]} --socket #{db_socket} -p\"#{db_pass.strip}\"  -e 'SHOW SLAVE STATUS\\G'"
				stdout, stderr, status = Open3.capture3(cmd)
				if status.to_i != 0
					critical "Error message: status: #{status}"
				end
				...
				table.keys.to_a.each do |k|
					['Slave_IO_State', 'Slave_IO_Running', 'Slave_SQL_Running', 'Last_IO_Error', 'Last_SQL_Error', 'Seconds_Behind_Master'].each do |key|
						dict.push("#{k.strip}") if key.strip == k.strip
					end
				end
